...
import { normalizeFormDataToArray } from '../utils/form-data.util';
...
export class WorkflowNodeService {
   ...
   */
  async setFormDataValue(node: WorkflowNode, parameterId: string, value: string, multiValueParamIds: Set<string>): Promise<WorkflowNode> {
    ...
    const updated = await this.workflowNodeModel.findOneAndUpdate({ _id: node._id }, { $set: { formData: entries } }, { new: true });
