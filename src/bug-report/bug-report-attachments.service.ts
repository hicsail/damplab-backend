...
export interface BugPresignedUploadRequest {
  bugId: string;
  filename: string;
  contentType: string;
  size: number;
...
export interface BugPresignedUploadResponse {
  filename: string;
  uploadUrl: string;
  key: string;
  contentType: string;
...
@Injectable()
export class BugReportAttachmentsService {
  ...
  async createPresignedUpload(request: BugPresignedUploadRequest): Promise<BugPresignedUploadResponse> {
    try {
      ...
      const key = `bugs/${request.bugId}/attachments/${uuidv4()}-${safeFilename}`;
      ...
      console.error('Failed to create S3 presigned URL for bug attachment', err);
      throw new InternalServerErrorException('Could not prepare upload URL for bug attachment');
    }
  }
  ...
  async createPresignedDownload(key: string, contentType?: string): Promise<string | null> {
    try {
      ...
      // eslint-disable-next-line no-console
      console.error('Failed to create S3 presigned download URL for bug attachment', err);
      return null;
    }
  }
}
